...
import { 
  FiUsers, FiActivity, FiServer, FiPlus, FiMoreVertical, 
  FiTrendingUp, FiShield, FiClock, FiLoader
} from "react-icons/fi";
import { FaPeopleGroup, FaUserDoctor } from "react-icons/fa6";
import IntroDisclosureDemo from "../../draw";
  ...
  if (isLoading) {
    return (
      <div className="w-full min-h-[80vh] flex flex-col items-center justify-center bg-[#FDFBF7] rounded-[2rem]">
        ...
        <h2 className="text-xl font-bold text-slate-800">Loading Command Center...</h2>
        <p className="text-sm text-slate-500">Syncing with NeuroCast Database</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-h-screen bg-[#FDFBF7] text-slate-800 font-sans p-4 sm:p-8 rounded-[2rem] overflow-hidden flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Predictions" value={data.stats.totalPredictions.toLocaleString()} trend="+14.2%" icon={<FiActivity />} color="text-blue-600" bg="bg-blue-50" />
        <StatCard title="Active Patients" value={data.stats.activePatients.toLocaleString()} trend="+5.1%" icon={<FiUsers />} color="text-emerald-600" bg="bg-emerald-50" />
        <StatCard title="API Uptime" value={data.stats.apiUptime} trend="Vercel Edge" icon={<FiServer />} color="text-violet-600" bg="bg-violet-50" />
        <StatCard title="Security Status" value={data.stats.securityStatus} trend="HIPAA Compliant" icon={<FiShield />} color="text-amber-600" bg="bg-amber-50" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm flex flex-col text-xs">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-lg font-bold text-slate-800">Platform Traffic & API Calls</h2>
              <p className="text-xs text-slate-400 font-medium">Last 7 days (Vercel Analytics Simulation)</p>
            </div>
            <div className="flex items-center gap-4 text-xs font-bold">
              <span className="flex items-center gap-1.5 text-slate-600">
                <div className="w-2 h-2 rounded-full bg-slate-800"></div> Web Visits
              </span>
              <span className="flex items-center gap-1.5 text-blue-600">
                <div className="w-2 h-2 rounded-full bg-blue-500"></div> AI Inferences
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 w-full h-[250px] min-h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.trafficData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                <defs>
                ...
                />
                <Area type="monotone" dataKey="visits" stroke="#1E293B" strokeWidth={3} fillOpacity={1} fill="url(#colorVisits)" />
                <Area type="monotone" dataKey="api_calls" stroke="#3B82F6" strokeWidth={3} fillOpacity={1} fill="url(#colorApi)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm flex flex-col items-center text-xs">
          <div className="w-full flex justify-between items-center mb-2">
            <div>
              ...
              <p className="text-xs text-slate-400 font-medium">Global Patient Cohort</p>
            </div>
          </div>
          ...
          <div className="relative w-full flex-1 flex items-center justify-center min-h-[200px]">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-3xl font-black text-slate-800">
                {data.riskData.reduce((acc, curr) => acc + curr.value, 0).toLocaleString()}
              </span>
              <span className="text-[10px] uppercase tracking-wider font-bold text-slate-400">Patients</span>
            </div>
          </div>
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm overflow-hidden flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-bold text-slate-800">Recent AI Stratifications</h2>
            <button className="text-blue-600 text-sm font-bold hover:underline">View Database</button>
          </div>
                    ...
                    <td className="py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 font-mono">{patient.id} • {patient.age}y</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 text-sm font-mono text-slate-600">{patient.mmse} / 30</td>
                    <td className="py-3">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${
                        patient.risk === "High Risk" || patient.risk === "Critical" ? "bg-red-50 text-red-600 border border-red-100" :
                        patient.risk === "Moderate" ? "bg-amber-50 text-amber-600 border border-amber-100" :
        ...
        <div className="lg:col-span-4 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl text-white flex flex-col">
          ...
          <div className="flex flex-col gap-3 flex-1 justify-center">
            {data.doctorsList.map((doc, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 rounded-2xl hover:bg-slate-800 transition-colors cursor-pointer border border-transparent hover:border-slate-700">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 uppercase tracking-wider">{doc.spec}</p>
                  </div>
                </div>
...
function StatCard({ title, value, trend, icon, color, bg }: { title: string, value: string | number, trend: string, icon: React.ReactNode, color: string, bg: string }) {
  return (
