...
type FileStatus = "idle" | "dragging" | "uploading" | "error" | "complete";
...
export default function FileUpload({
    ...
    (uploadingFile: File) => {
      ...
      uploadIntervalRef.current = setInterval(() => {
        ...
        if (currentProgress >= 100) {
          ...
          setProgress(100); // Keep progress at 100
          setStatus("complete"); 
  ...
  const handleFileSelect = useCallback(
    (selectedFile: File | null) => {
      ...
      // *** CRITICAL: Send file to Parent (Hero.js) ***
      if (onFileSelect) {
    ...
    [handleError, validateFileSize, validateFileType, validateFile, simulateUpload, onFileSelect]
  );
              ...
              <AnimatePresence mode="wait">
                {status === "idle" || status === "dragging" ? (
                  ...
                  >
                    ...
                    <div className="mb-4 space-y-1.5 text-center">
                      <h3 className="truncate font-semibold text-blue-500 text-sm dark:text-white ">
                        {file?.name}
                      </h3>
                ...
                {status === "complete" && file ? (
                  <motion.div
                    ...
                    className="absolute inset-0 flex flex-col items-center justify-center p-6"
                  >
                    <div className="mb-4 rounded-full bg-green-500/10 p-3 dark:bg-green-500/20">
                      <BsFileCheck className="text-green-400" />
                    </div>

                    <div className="mb-4 space-y-1.5 text-center">
                      <h3 className="truncate font-semibold text-gray-900 text-sm dark:text-white max-w-xs">
                        {file.name}
                      </h3>
                      <div className="flex items-center justify-center gap-2 text-xs">
                        <span className="text-gray-500 dark:text-gray-400">
                    ...
                    <button
                      type="button"
                      onClick={resetState}
                      className="group flex w-4/5 items-center justify-center gap-2 rounded-lg bg-red-50 px-4 py-2.5 font-semibold text-red-600 text-sm transition-all duration-200 hover:bg-red-100 dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/20"
                    >
                      Remove File
                    </button>
                  </motion.div>
              ...
              {error && (
                <>
                  <motion.div
                    ...
                    className="-translate-x-1/2 absolute bottom-4 left-1/2 transform rounded-lg border border-red-500/20 bg-red-500/10 px-4 py-2 w-[90%] text-center"
                    exit={{ opacity: 0, y: -10 }}
                    initial={{ opacity: 0, y: 10 }}
                  >
